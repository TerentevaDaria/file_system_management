...
{
    public FileMoveParser(IFileSystemController controller)
        : base(controller)
    {
    }
    ...
    public override ICommand? TryParsing(IReadOnlyList<string> text)
    {
        ...
        textEnumerator.MoveNext();
        const string commandNamePartTwo = "move";
        ...
        return new FileMove(Controller, new Path(sourcePath), new Path(destinationPath));
    }
}